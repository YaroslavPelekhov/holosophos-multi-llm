...
├── tests/                 # Тесты
├── Dockerfile
├── world_model_experiments/ # Экспериментальный модуль
├── docker-compose.yml
├── pyproject.toml
└── README.md
```
...
---
Экспериментальный модуль содержит код для проведения контролируемых экспериментов, используемых при генерации и анализе научных статей по направлению **«обучение с подкреплением с использованием генеративных моделей мира»**.

Эксперименты ориентированы на исследование **ограничений и failure modes** латентных world model-подходов (Dreamer-подобные архитектуры), включая:
- деградацию качества планирования при увеличении горизонта,
- влияние частичной наблюдаемости,
- ошибки, возникающие при использовании латентных пространств действий.

Результаты экспериментов (метрики, таблицы, данные для графиков) сохраняются в директории `world_model_experiments/results/` и используются агентами Holosophos при формировании аналитических разделов и обсуждения в сгенерированных научных текстах.
___
## Настройка окружения
